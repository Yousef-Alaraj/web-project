...
const place = jordanDestinations.find(destination => destination.id === Number(placeId));


if(place){
    const titleElement = document.getElementById("title");
    titleElement.innerText = place.name;
    
    const placeTitle = document.getElementById("place-title");
    placeTitle.innerText = place.name;

    const placeCategory = document.getElementById("place-category");
    placeCategory.innerText = place.categoryName;

    const placeRating = document.getElementById("place-rating");
    placeRating.innerText = place.rating;

    const galleryTitleName = document.getElementById("gallery-title-name");
    galleryTitleName.innerText = place.name;

    const galleryContainer = document.getElementById("place-gallery");
    galleryContainer.innerHTML = "";

    place.images.forEach(source => {
        const imageHTML = `<img src="${source}" alt="Picture of ${place.name}">`;
    ...
    infoLocation.innerText = place.location;

    const infoHours = document.getElementById("info-duration");
    infoHours.innerText = place.tripDuration;

    const infoFee = document.getElementById("info-fee");
    infoFee.innerText = place.fee;

    const infoContact = document.getElementById("info-contact");
    infoContact.innerText = place.contactNumber;

    const infoTags = document.getElementById("info-tags");
    infoTags.innerText = place.tags.join(", ");
